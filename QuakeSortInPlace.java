...
public class QuakeSortInPlace {
    ...
    public void onePassBubbleSort(ArrayList<QuakeEntry> quakeData , int numSort){
        for(int i = 0 ; i < quakeData.size() - 1 - numSort; i++){
            if(quakeData.get(i).getMagnitude() > quakeData.get(i + 1).getMagnitude()){
                QuakeEntry ai = quakeData.get(i);
                QuakeEntry an = quakeData.get(i+1);
                
                quakeData.set(i , an);
                quakeData.set(i+1 , ai);
            }
        }
    }
    ...
    public void sortByLargestDepth(ArrayList<QuakeEntry> quakeData){
        ...
        for(int i = 0 ; i < quakeData.size() ; i++){
            int laridx = getLargestDepth(quakeData , i);
            QuakeEntry qi = quakeData.get(i);
            QuakeEntry qn = quakeData.get(laridx);
            quakeData.set(i , qn);
            quakeData.set(laridx , qi);
            numpasses += 1;
            if(numpasses == 50){
                break;
            }
